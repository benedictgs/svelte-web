...
<IntersectionObserver let:intersecting once={false}>
    {#if intersecting}
        <section id={id} class="flex justify-center bg-cover bg-top bg-no-repeat bg-fixed bg-[url('$lib/media/background-hero-blur.webp')] pt-4 ">
            <div class="w-full lg:max-w-screen-lg mb-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 lg:gap-10 bg-black/50  mx-3 p-6 md:px-6 rounded-xl">
                <div class="flex flex-col justify-between sm:max-w-sm row-span-2 lg:col-span-1">
                    <div in:fade={{delay:200,duration:1000}} class="flex flex-row hover:animate-pulse">
                    ...
                    <picture in:fly="{{delay:500,duration:1000, y:50}}" class=" hidden sm:grid justify-center hover:animate-pulse">
                        <img loading="lazy" src={PotraitProfile} alt="Profile" class="max-h-[40rem] ">
                    </picture>
                    <div in:fly="{{delay:700,duration:2000, y:50}}" id="stats" class="grid grid-cols-3 place-items-center p-3">
                ...
                <div>
                    ...
                    <h3 in:fade="{{delay:500, duration:1000}}" class="font-bebas text-xl hover:animate-pulse">Personal Details</h3>
                    <ul in:fly="{{delay:700,duration:1000, y:50}}" class="">
                        {#each stats as stat}
                ...
                <div class="flex flex-col">
                    <hr class=" my-3  border-gray-600">
                    <h3 in:fade="{{delay:500,duration:1000}}" class="font-bebas text-xl hover:animate-pulse">Chart</h3>
                    <div in:fly="{{delay:700,duration:1000, y:50}}" class=" min-h-[10rem] hover:animate-pulse">
                        <RadarContainer/>
                    </div>
                </div>
                <div class="sm:col-span-2 flex flex-col">
                    ...
                    <div in:fly="{{delay:1400,duration:1000, y:50}}" class="min-h-[10rem] hover:animate-pulse">
                        <LineAreaContainer/>
                    </div>
                </div>
    ...
    {:else}
    <section id={id} class="flex justify-center bg-cover bg-top bg-no-repeat bg-fixed bg-[url('$lib/media/background-hero-blur.webp')] pt-4 max-h-96"></section>
    {/if}
